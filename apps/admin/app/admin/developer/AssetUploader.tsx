...
export function AssetUploader({
  ...
  async function handleFile(file: File) {
    ...
    if (!ACCEPTED.includes(file.type)) {
      setError("Use PNG, JPEG, WebP, SVG, or ICO.");
      return;
    }
    ...
    <div className="rounded-xl border border-white/5 p-4">
      ...
      <div className="relative h-24 w-full rounded-lg bg-charcoal-800 overflow-hidden mb-3 flex items-center justify-center">
        {preview ? (
          ...
          <span className="text-xs text-smoke-500">No asset set</span>
        )}
      </div>
      <input
        ref={inputRef}
      ...
      >
        {isUploading ? "Uploading…" : <><Upload size={14} /> {currentUrl ? "Replace" : "Upload"}</>}
      </button>
