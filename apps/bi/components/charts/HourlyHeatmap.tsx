  ...
  return (
    <div className="glass-panel p-4 overflow-x-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-medium text-charcoal-100">Peak order hours</h3>
        {hover && (
          <p className="text-xs text-smoke-400">
            {hover.dow} {hover.hour}:00 — {hover.orderCount} orders
          </p>
        )}
      </div>
      <div className="min-w-[560px]">
        ...
        {DOW_ORDER.map((dow) => (
          <div key={dow} className="grid grid-cols-[32px_repeat(24,1fr)] gap-[2px] mb-[2px]">
            <div className="text-[10px] text-smoke-500 flex items-center">{dow}</div>
            {HOURS.map((hour) => {
