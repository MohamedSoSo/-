...
// Owner BI is management-only (owner/developer) — no staff, no customers.
// Financial/supplier-cost data lives here, so this is stricter than the POS
// app's "any staff" gate.
export async function updateSession(request: NextRequest) {
  ...
  if (!isPublic && user) {
    ...
    if (!profile || !["owner", "developer"].includes(profile.role)) {
      const url = request.nextUrl.clone();
      url.pathname = `/${locale}/login`;
      url.searchParams.set("error", "not_management");
      return withSecurityHeaders(NextResponse.redirect(url), nonce);
    }
  }
