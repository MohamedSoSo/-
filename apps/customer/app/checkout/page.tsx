...
export default function CheckoutPage() {
  ...
  async function placeOrder() {
    ...
    if (!payload.success) {
      setError(payload.error.issues[0]?.message ?? "Please check your order details.");
      return;
    }
  ...
  if (items.length === 0) {
    return (
      <main className="min-h-screen flex items-center justify-center px-6 text-center">
        <div>
          <p className="text-charcoal-100">Your cart is empty.</p>
    ...
    <main className="max-w-lg mx-auto px-4 py-8 pb-32">
      ...
      {isDineIn ? (
        <div className="glass-panel p-4 mb-6">
          <p className="text-white font-medium">Dine-in — Table {tableLabel}</p>
          <p className="text-sm text-smoke-400">Locked from your table's QR code.</p>
        </div>
      ) : (
        <section className="mb-6">
          <h2 className="text-sm font-medium text-charcoal-100 mb-2">Order type</h2>
          <div className="grid grid-cols-3 gap-2">
            {(
              [
                { mode: "pickup" as OrderMode, label: "Pickup" },
                { mode: "delivery" as OrderMode, label: "Delivery" },
                ...(flags.pre_orders ? [{ mode: "pre_order" as OrderMode, label: "Pre-order" }] : []),
              ]
            ).map((opt) => (
              <button
                key={opt.mode}
      ...
      {orderMode === "delivery" && !isDineIn && (
        <section className="mb-6">
          <h2 className="text-sm font-medium text-charcoal-100 mb-2">Delivery address</h2>
          <textarea
            value={address}
      ...
      {orderMode === "pre_order" && !isDineIn && (
        <section className="mb-6">
          <h2 className="text-sm font-medium text-charcoal-100 mb-2">Schedule</h2>
      ...
      {!isDineIn && (
        <section className="mb-6">
          <h2 className="text-sm font-medium text-charcoal-100 mb-2">Payment</h2>
          <div className="space-y-2">
            {(
              [
                { method: "card" as PaymentMethod, label: "Card (Visa/Mastercard)" },
                { method: "apple_pay" as PaymentMethod, label: "Apple Pay" },
                { method: "cash_on_delivery" as PaymentMethod, label: "Cash / Terminal on delivery" },
              ]
            ).map((opt) => (
              <button
                key={opt.method}
                onClick={() => setPaymentMethod(opt.method)}
                className={`w-full text-left rounded-xl2 border px-4 py-2.5 text-sm transition-colors ${
      ...
      <Button variant="primary" size="lg" className="w-full" disabled={isPlacing} onClick={placeOrder}>
        {isPlacing ? "Placing order…" : `Place order — ${total.toFixed(2)} SAR`}
      </Button>
    </main>
  );
}
...
function friendlyRpcError(message?: string): string {
  ...
  if (message.includes("OUT_OF_STOCK")) return "One of your items just sold out — please update your cart.";
  if (message.includes("ITEM_UNAVAILABLE")) return "One of your items is no longer available.";
  if (message.includes("TABLE_REQUIRED")) return "Please scan your table's QR code to order dine-in.";
